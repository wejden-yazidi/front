...
export class CompteBancaireService {
  ...
  createCompteBancaire(compteBancaire: CompteBancaire) {
    return this.httpCompteBancaire.post(
      this.urlCreateCompteBancaire,
      compteBancaire
    );
  }
  updateCompteBancaire(id: string, data: any) {
    return this.httpCompteBancaire.put(
      `${this.urlUpdateCompteBancaire}/${id}`,
      data
    );
  }
  deleteCompteBancaire(id: string) {
    return this.httpCompteBancaire.delete(
      `${this.urlDeleteCompteBancaire}/${id}`
    );
  }
  getCompteBancaireId(id: string) {
    return this.getAllCompteBancaire().pipe(
      map((res) => res.find((c) => c.id == id))
    );
  }
}
